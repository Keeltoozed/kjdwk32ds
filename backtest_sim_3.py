...
def test_strategy(prices):
    ...
    for t, p in enumerate(prices):
        ...
        # Smart Break-Even: If it hits +25%, lock stop at +5% (covers fees + small profit)
        if max_pnl >= 0.25:
            current_stop = 0.05 
...
max_win = np.max(pnls)
print(f"Smart Break-Even (+25% -> +5%): WinRate={win_rate*100:.1f}%, AvgPnL={avg_pnl*100:.1f}%, MaxWin={max_win*100:.1f}%")
