...
bad_logic = '''            buys = [t for t in state.trades if t['type'] == 'buy']
...
good_logic = '''            buy_count = sum(1 for t in state.trades if t['type'] == 'buy')
...
content = content.replace("            df['volume_buy'] = np.where(df['type'] == 'buy', df['curve_sol_diff'].abs(), 0)\n            df['volume_sell']", "            df['volume_sell']")
